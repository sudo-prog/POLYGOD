...
try:
    mem0_config = json.loads(settings.MEM0_CONFIG) if settings.MEM0_CONFIG else {"vector_store": {"provider": "qdrant", "url": "http://qdrant:6333"}}
...
class MicroNicheScanner:
    ...
    async def scan_niches(self, mode: int = 1) -> List[Dict[str, Any]]:
        ...
        niche_keywords = ["weather", "temperature", "rain", "snow", "tweet", "post", "mention", "elon", "twitter"]
        markets = [
            m for m in all_markets
            if any(kw in m.get("title", "").lower() or kw in m.get("slug", "").lower() for kw in niche_keywords)
            or m.get("liquidity", 0) < 5000  # low-liquidity edge
        ]
        
        logger.info(f"Found {len(markets)} potential niche markets out of {len(all_markets)} total")
        ...
        for market in markets:
            try:
                ...
                if "weather" in title_lower or "temperature" in title_lower:
                    forecast = await self.get_weather_forecast(market.get("city", "New York"))
                    edge = self.calculate_weather_edge(forecast, prices)
                    niche_type = "weather"
                    
                elif "tweet" in title_lower or "post" in title_lower or "elon" in title_lower:
                ...
                if edge > 0.20:  # threshold for actionable edge
                    ...
                    logger.info(f"Edge found: {niche_type} | {market.get('title', '')[:50]}... | Edge: {edge:.2%}")
                    
            except Exception as e:
                logger.warning(f"Error scanning market {market.get('id', 'unknown')}: {e}")
                continue
        ...
        logger.info(f"MICRO NICHE SCANNER COMPLETE: Found {len(opportunities)} opportunities")
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if temps:
                    ...
                    return [(t - min_temp) / range_temp for t in temps[:24]]  # first 24 hours
                return [0.5] * 24
        except Exception as e:
    ...
    def calculate_weather_edge(self, forecast: List[float], market_prices: Dict[str, float]) -> float:
        """
        ...
        forecast_prob = sum(forecast[:6]) / len(forecast[:6]) if len(forecast) >= 6 else forecast[0]
    ...
    def calculate_tweet_edge(self, count_data: Dict[str, Any], market_prices: Dict[str, float]) -> float:
        """
        ...
        for bucket, price in market_prices.items():
            try:
                ...
                bucket_lower = int(bucket.split("-")[0]) if "-" in bucket else int(bucket)
        ...
        for opp in opps:
            try:
                ...
                state = {
                    ...
                    "debate_history": [
                        {"agent": "MicroNicheScanner", "output": f"Edge: {opp['edge']:.2%} in {opp['niche']} niche"}
                    ],
                }
                ...
                logger.error(f"Tournament failed for {opp.get('market_id', 'unknown')}: {e}")
                mem0_add(
