"""initial core tables - markets, news_articles, price_history, app_state, llm_providers, agent_configs, usage_logs
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("liquidity", sa.Float, default=0.0),
        sa.Column("yes_percentage", sa.Float, default=50.0),
        sa.Column("is_active", sa.Boolean, default=True),
        sa.Column("end_date", sa.DateTime(timezone=True), nullable=True),
        sa.Column("image_url", sa.Text, nullable=True),
        sa.Column("clob_token_ids", sa.Text, nullable=True),
        sa.Column("last_updated", sa.DateTime(timezone=True), server_default=sa.text("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')"), onupdate=sa.text("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')")),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')")),
    )
    ...
    op.create_table(
        ...
        sa.Column("timestamp", sa.DateTime(timezone=True), server_default=sa.text("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')"), nullable=False),
    )
    op.create_index("idx_price_history_market_time", "price_history", ["market_id", "timestamp"])
    ...
    op.create_index("idx_news_market_published", "news_articles", ["market_id", "published_at"])
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')"), onupdate=sa.text("(CURRENT_TIMESTAMP AT TIME ZONE 'UTC')")),
    )
    ...
    op.create_table(
        ...
        sa.Column("uptime_24h", sa.String(20), default="100%"),
        sa.Column("avg_speed", sa.Integer, nullable=True),
        sa.Column("tokens_today", sa.Integer, default=0),
    ...
    op.create_index("idx_usage_logs_provider_timestamp", "usage_logs", ["provider", "timestamp"])
