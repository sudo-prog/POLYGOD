...
|------------|--------|-----------------------------------------------------------|
| 2026-04-17 | Kilo (GOD TIER ENGINEER) | Implemented AutoDog daily situational awareness system: automated 9am AEST digest via Telegram with system load, 24h trading activity, current POLYGOD mode, recent evolution events, and top market opportunities. Added /autodog on/off command for enabling/disabling auto-reporting and /report command for instant digest generation. Background task scheduled with APScheduler timezone support. Comprehensive digest pulls data from system monitoring, database queries, and niche scanner. Added AUTODOG_ENABLED flag for runtime control. |
