    ...
    <div className="flex flex-col space-y-6 animate-fadeIn">
              ...
              >
                ...
                <p
                  className={`text-xs font-medium ${
                    isEnabled ? colorClasses.text : 'text-gray-500'
                  }`}
                >
                  {agent.label}
                </p>
                <p className="text-[10px] text-gray-500 mt-0.5 line-clamp-1">{agent.description}</p>
              </button>
            );
          })}
        </div>
      ...
      <div className="space-y-4">
        {messages.map((msg, idx) => (
          <div
            key={idx}
            className={`p-5 rounded-2xl border backdrop-blur-sm transition-all duration-500 animate-slideInLeft ${
